                  ...
                  <v-table>
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="student-name-value">{{ currentProfile?.profile.first_name }} {{
                          currentProfile.profile.last_name }}</td>
                      </tr>
                    </tbody>
                  </v-table>
                </v-card>
...
async function initialize() {
  try {
    ...
    const { data: gate_history } = await useFetch('/api/getGateHistory')
    if (gate_history) {
      historyList.value = gate_history.value;
    }
  } catch (err) {
...
async function searchCardProfile(id) {
  ...
  console.log(cards);
    ...
    currentPhoto.value = base_url + cards[0].profile.image_url;
    return cards;
  }
}
...
async function handleSubmit() {
  ...
  const payload = {
    ...
    profile_avatar: myProfileDetails[0].profile.image_url
  }
