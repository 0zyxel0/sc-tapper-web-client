  ...
  <div>
            ...
            <v-card elevation="0">
              ...
              <v-card-text>
                <p class="font-weight-bold"><v-icon aria-hidden="false" size="30"
                    color="primary">mdi-card-account-details</v-icon> Card Details
                </p>
                ...
                <div class="mt-3" v-if="profileDetails.is_card_assign == true">
                  <v-row dense>
                    ...
                    <v-col cols="8">
                      <p v-if="cardDetails.is_active == true">
                        <v-badge dot color="success" inline></v-badge>Active
                      </p>
                      <p v-else>
                        <v-badge dot color="error" inline></v-badge>Inactive
                      </p>
                    </v-col>
                  </v-row>
                </div>
              ...
              <v-divider></v-divider>
              <v-card-actions>
                <v-btn v-if="profileDetails.is_card_assign == true" prepend-icon="mdi-link-off" class="my-2" color="red"
                  variant="outlined" block @click="showUnlinkDialogBox">Unlink</v-btn>

                <v-btn v-else prepend-icon="mdi-link-plus" class="my-2" color="primary" variant="outlined" block
                  @click="assignCardDialog = true">Link
                  Card</v-btn>
              </v-card-actions>
            </v-card>
          </v-col>
    ...
    <v-dialog v-model="updateImageDialogbox" persistent max-width="300">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-file-input :rules="rules.photo" v-model="avatarImage" accept="image/png, image/jpeg, image/bmp"
            density="compact" prepend-icon="mdi-camera" label="Upload Image" ref="uploader" required class="d-none"
            @change="onFileChange" @click:clear="clearImagePreview()"></v-file-input>
        ...
        <v-card-actions class="justify-center">
          <v-btn color="primary" class="text-none mt-2" block round variant="elevated" prepend-icon="mdi-cloud-upload"
            @click="uploadUpdatedImage">
            Upload
          </v-btn>
    ...
    <v-dialog v-model="deleteProfileDialog" width="auto">
      <v-card max-width="400" prepend-icon="mdi-delete-alert" color="blue-grey-darken-4"
        ...
        <template v-slot:actions>
          <v-btn variant="tonal" :loading="loading" text="Delete" @click="deleteProfile" prepend-icon="mdi-delete"
            color="red"></v-btn>
...
async function initialize() {
  try {
    let { data: profile_details, error } = await $fetch(`/api/profile/${route.params.id}`)
    if (profile_details) {
      profileDetails.value = profile_details[0];
      cardDetails.value = profile_details[0].card_details;
      profileid.value = profile_details[0].id;
      profile_publicid.value = profile_details[0].publicid

      student_no.value = profile_details[0].studentno
      last_name.value = profile_details[0].last_name
      first_name.value = profile_details[0].first_name
      middle_name.value = profile_details[0].middle_name
      image_id.value = profile_details[0].image_id
      publicID.value = profile_details[0].publicid
      //console.log("Image ID: ", image_id.value)
